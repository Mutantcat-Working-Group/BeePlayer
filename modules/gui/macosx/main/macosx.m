...
#define RECENT_ITEMS_LONGTEXT N_("By default, BeePlayer keeps a list of the last 10 items. " \
                                 "This feature can be disabled here.")

#define USE_APPLE_REMOTE_TEXT N_("Control playback with the Apple Remote")
#define USE_APPLE_REMOTE_LONGTEXT N_("By default, BeePlayer can be remotely controlled with the Apple Remote.")

#define USE_APPLE_REMOTE_VOLUME_TEXT N_("Control system volume with the Apple Remote")
#define USE_APPLE_REMOTE_VOLUME_LONGTEXT N_("By default, BeePlayer will control its own volume with the Apple Remote. However, you can choose to control the global system volume instead.")

#define DISPLAY_STATUS_ICONMENU_TEXT N_("Display BeePlayer status menu icon")
#define DISPLAY_STATUS_ICONMENU_LONGTEXT N_("By default, BeePlayer will show the statusbar icon menu. However, you can choose to disable it (restart required).")

#define USE_APPLE_REMOTE_PREVNEXT_TEXT N_("Control play queue items with the Apple Remote")
#define USE_APPLE_REMOTE_PREVNEXT_LONGTEXT N_("By default, BeePlayer will allow you to switch to the next or previous item with the Apple Remote. You can disable this behavior with this option.")

#define USE_MEDIAKEYS_TEXT N_("Control playback with media keys")
#define USE_MEDIAKEYS_LONGTEXT N_("By default, BeePlayer can be controlled using the media keys on modern Apple " \
                                  "keyboards.")

#define NATIVE_FULLSCREEN_MODE_ON_LION_TEXT N_("Use the native fullscreen mode")
#define NATIVE_FULLSCREEN_MODE_ON_LION_LONGTEXT N_("By default, BeePlayer uses the fullscreen mode known from previous Mac OS X releases. It can also use the native fullscreen mode on Mac OS X 10.7 and later.")
...
#define ITUNES_LONGTEXT N_("BeePlayer will pause and resume supported music players on playback.")

#define AUTOLOAD_EXTENSIONS_TEXT N_("Automatically load extensions on startup")
#define AUTOLOAD_EXTENSIONS_LONGTEXT N_("Automatically load and enable BeePlayer extensions when the application starts.")
...
#define CONTINUE_PLAYBACK_LONGTEXT N_("BeePlayer will store playback positions of the last 30 items you played. If you re-open one of those, playback will continue.")
